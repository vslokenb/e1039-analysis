...
#include "CalibDriftDistMod.h"
using namespace std;

CalibDriftDistMod::CalibDriftDistMod(const std::string& name)
  : SubsysReco(name)
...
CalibDriftDistMod::~CalibDriftDistMod()
{
  if (m_cal_xt ) delete m_cal_xt ;
}

int CalibDriftDistMod::Init(PHCompositeNode* topNode)
{
...
int CalibDriftDistMod::InitRun(PHCompositeNode* topNode)
{
  ...
  if (m_reso_d0 > 0) {
    if (Verbosity() > 0) cout << "CalibDriftDistMod: Set the plane resolution.\n";
...
int CalibDriftDistMod::process_event(PHCompositeNode* topNode)
{
  GeomSvc* geom = GeomSvc::instance();
  for (SQHitVector::Iter it = m_vec_hit->begin(); it != m_vec_hit->end(); it++) {
    ...
    if (! det) {
      cerr << "  WARNING:  Cannot find the in-time parameter for det=" << det << " in CalibDriftDistMod.\n";
      continue;
...
int CalibDriftDistMod::End(PHCompositeNode* topNode)
{
  return Fun4AllReturnCodes::EVENT_OK;
}
 ...
 */
void CalibDriftDistMod::SetResolution(const double reso_d0, const double reso_d1, const double reso_d2, const double reso_d3p, const double reso_d3m)
{
  m_reso_d0  = reso_d0 ;
  m_reso_d1  = reso_d1 ;
  m_reso_d2  = reso_d2 ;
