...
DIR_OUT_BASE=/data4/e1039_data/online/sraw_recal
N_PROC=4
...
function ProcessOneFile {
    ...
    local -r  FN_OUT=$DIR_OUT/run_${RUN6}_spill_${SPILL9}_sraw.root
    ...
    echo 
    mkdir -p $DIR_OUT
    root.exe -b -q "Fun4All.C($RUN, $SPILL, \"$DST\", \"$FN_OUT\")" &>$FN_LOG
}
